...
// 原文代码中,有个late关键词,用于dart null safety.
...
void newLine() {
  ...
  SetConsoleCursorPosition(stdout, bufferInfo.dwCursorPositionY << 16 + bufferInfo.dwCursorPositionX);
}
...
void scrollScreenBuffer(int handle, int x) {
  ...
  ScrollConsoleScreenBuffer(
      handle, scrollRect.addressOf, clipRect.addressOf, coordDest.Y << 16 + coordDest.X, fillChar.addressOf);
...
void main() {
  ...
  final mode = originalConsoleMode.value & ~(ENABLE_LINE_INPUT | ENABLE_ECHO_INPUT);
  SetConsoleMode(stdin, mode);

  newLine();

  while (true) {
    WriteFile(stdout, toCString(echoOffPrompt), echoOffPrompt.length, cWritten, nullptr);
